    ...
    try {
      ...
      if (response.ok) { let data = await response.json(); let locations = [];
        ...
        if (locations.length > 0) {
          ...
          let distanceApiUrl = `https://fourth-basin-develop-dem.trycloudflare.com/api/distance?customerlocation=${customerLocation}&destinations=${locations.join("|")}&shop=${document.domain}`;
